...
def air_time_reward(
    ...
    reward = torch.where(
        torch.logical_or(cmd > 0.0, body_vel > velocity_threshold),#change cmd threshold to 0.1
...
def individual_foot_clearance_reward(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, target_height: float, std: float, tanh_mult: float
) -> torch.Tensor:
    """Reward EACH foot individually for clearance - all must lift"""
    asset: RigidObject = env.scene[asset_cfg.name]
    foot_z_target_error = torch.square(asset.data.body_pos_w[:, asset_cfg.body_ids, 2] - target_height)
    foot_velocity_tanh = torch.tanh(tanh_mult * torch.norm(asset.data.body_lin_vel_w[:, asset_cfg.body_ids, :2], dim=2))
    reward_per_foot = torch.exp(-foot_z_target_error * foot_velocity_tanh / std)
    
    # MINIMUM across all feet (all must lift to get reward)
    return torch.min(reward_per_foot, dim=1)[0]
...
def joint_position_penalty(
    ...
    return torch.where(torch.logical_or(cmd > 0.0, body_vel > velocity_threshold), reward, stand_still_scale * reward) #change cmd threshold to 0.1
...
class ActionHistoryObs(ManagerTermBase):
    """Observation term providing a sliding window of past actions."""

    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.num_actions = env.action_manager.total_action_dim
        self.history_length = cfg.params.get("history_length", 4)
        self.buffer = torch.zeros(
            env.num_envs, self.history_length * self.num_actions,
            device=env.device
        )

    def __call__(self, env, history_length=4):
        self.buffer[:, :-self.num_actions] = self.buffer[:, self.num_actions:].clone()
        self.buffer[:, -self.num_actions:] = env.action_manager.action
        return self.buffer